...
class AutomationScheduler:
    ...
    def add_job(self, job_id: str, job_data: dict):
        ...
        async def task_func():
             print(f"🚀 [Scheduler] Executing job: {job_id} ({job_data['name']})")
             
             # Hỗ trợ thực thi Script Python nếu có
             if job_data.get("task_type") == "script":
                 import subprocess
                 import sys
                 script_path = job_data.get("script_path")
                 if script_path and os.path.exists(script_path):
                     print(f"📂 [Scheduler] Running script: {script_path}")
                     # Chạy script trong một tiến trình con
                     subprocess.Popen([sys.executable, script_path])
                 else:
                     print(f"❌ [Scheduler] Script not found: {script_path}")
