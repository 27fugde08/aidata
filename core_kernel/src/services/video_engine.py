...
class VideoEngine:
    ...
    async def process_viral_shorts(self, url: str, video_id: str, callback=None):
        ...
        # Clean up: Xóa video gốc để tiết kiệm bộ nhớ
        try:
            if os.path.exists(video_path):
                os.remove(video_path)
                print(f"🧹 [VideoEngine] Cleaned up source video: {video_path}")
        except Exception as e:
            print(f"⚠️ [VideoEngine] Could not delete source video: {str(e)}")
